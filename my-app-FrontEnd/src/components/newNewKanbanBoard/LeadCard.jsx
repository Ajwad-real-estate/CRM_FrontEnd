...
import { Card, CardContent, Typography, Chip, Button, Box, useTheme, IconButton } from '@mui/material';
...
const LeadCard = ({ lead }) => {
  ...
  return (
    <Card sx={{ padding: 2, marginBottom: 2, borderRadius: 2, boxShadow: 10, background: colors.columns[100] }}>
      <CardContent>
        <Typography variant="h5" sx={{ fontWeight: 'bold' }}>{lead.title}</Typography>
        ...
        <Typography variant="body2" sx={{ marginTop: 1 }}>
          Project: {typeof lead.amount === 'number' ? `$${lead.amount.toFixed(2)}` : ' Not provided'}
        </Typography>
            ...
            >
              ...
              {lead.tags && lead.tags.length > 0 ? (
                lead.tags.map((tag, index) => (
                  <Chip key={index} label={tag} icon={<TagIcon />} sx={{
                    // marginRight: 0.5,
                    pl: "5px", pr: '5px'
                  }} />
                ))
