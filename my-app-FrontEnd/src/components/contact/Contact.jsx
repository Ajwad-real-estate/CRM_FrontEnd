...
import {
    Box, TextField, IconButton, Popover, Typography, MenuItem, Divider, ListItemText,
    Grid, Chip
} from '@mui/material';
...
const ContactPage = () => {
    ...
    const companies = [...new Set(allLeads.map((lead) => lead.company).filter(Boolean))];
    const tags = [...new Set(allLeads.flatMap((lead) => lead.tags))];

    const filteredLeads = allLeads.filter((lead) => (
        (!searchTerm || lead.title.toLowerCase().includes(searchTerm.toLowerCase())) &&
            ...
            >
                <Box p={2} width={300}>
                    ...
                    <Box mt={2}>
                        ...
                        {['Individuals', 'Companies', 'Employees', 'Customer Invoices', 'Vendor Bills', 'Archived'].map((filter) => (
                            <MenuItem key={filter}>
                    ...
                    {/* Group By Section */}
                    <Box mt={2}>
                        <Typography variant="body2" color="textSecondary">Group By</Typography>
                        {['Salesperson', 'Company', 'Country', 'Add Custom Group'].map((group) => (
                            <MenuItem key={group}>
