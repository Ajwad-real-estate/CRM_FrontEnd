...
const Calendar = () => {
  ...
  // const { data: clientData } = useClient(selectedAction?.client_id || null);
  // console.log("clientData", clientData);
  console.log("actions actions", actions);
  // const { data: clients = [] } = useClients(); // Assume useClients fetches all clients
  const { formattedTasks, formattedActions } = useMemo(() => {
    ...
    const formatEvents = (items, type) =>
      items.map((item) => {
        // Fetch client data for each action
        const { data: clientData } = useClient(item.client_id);

        return {
          id: item?.id,
          client_id: item.client_id,
          title: item?.title || clientData?.name || "Action",
          clientData, // Include full client data in the event
          // ... rest of your existing event properties ...
        };
      });
  ...
  const EventContent = ({ event }) => {
    const { data: client } = useClient(event.extendedProps.client_id);
    ...
    return (
      <div
        ...
        {event.extendedProps.type === "action"
          ? client
            ? client.name
            : "Loading client..."
          : event.title}
