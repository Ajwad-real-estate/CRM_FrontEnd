...
const checkoutSchema = yup.object().shape({
    ...
    phoneNumbers: yup.array().of(yup.string().matches(phoneRegExp, "Phone number is not valid")).required("At least one phone number is required")
});
        ...
        const fetchRoles = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    toast.error("Failed to load roles.", { position: "top-center", autoClose: 5000 });
                }
            } catch (error) {
                toast.error("An error occurred while fetching roles.", { position: "top-center", autoClose: 5000 });
            }
        };
    ...
    const handleFormSubmit = async (values) => {
        ...
        try {
            ...
            if (response.status === 401) {
                toast.error("Unauthorized. Please logout and login again.", { position: "top-center", autoClose: 5000 });
            ...
            if (response.ok) {
                toast.success("Account created successfully!", { position: "top-center", autoClose: 5000 });
                setAddSound(true); // Play success sound
            } else {
                toast.error(`Error: ${result.message}`, { position: "top-center", autoClose: 5000 });
                setDltSound(true); // Play error sound
            }
        } catch (error) {
            toast.error("An unexpected error occurred. Please try again.", { position: "top-center", autoClose: 5000 });
            setDltSound(true); // Play error sound
        } finally {
            setLoading(false); // Set loading to true while waiting for response
                            ...
                            >
                                ...
                                {values.phoneNumbers.map((phoneNumber, index) => (
                                    <TextField
                                        ...
                                        error={!!touched.phoneNumbers && !!errors.phoneNumbers?.[index]}
                                        helperText={touched.phoneNumbers && errors.phoneNumbers?.[index]}
                                        sx={{ gridColumn: "span 4" }}
                                    />
                                ))}
