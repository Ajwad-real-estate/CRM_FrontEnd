...
import commissionRequests from "../../src/data/commissionRequests";
import pipelineData from "../../src/data/sampleData";
import {
  Box,
  Button,
  MenuItem,
...
const RequestCommissionPage = () => {
  ...
  const wonLeads =
    pipelineData.find((stage) => stage.id === "won")?.leads || [];
  const [selectedLead, setSelectedLead] = useState("");

  const totalWonValue = wonLeads.reduce((sum, lead) => {
    const numericValue = parseFloat(
      lead.value.replace(" LE", "").replace(",", "")
    );
  ...
  return (
    <Box className="request-commission-page">
      ...
      <Box className="ratio-box">
        ...
        <Typography>
          Total Won Value: {totalWonValue.toLocaleString()} LE
        </Typography>
      </Box>
      ...
      <Box className="requests-list" mt={4}>
        ...
        {requests.map((request) => (
          <Box key={request.id} className="request-item">
            <Typography>
              <strong>Agent:</strong> {request.agentName}
            </Typography>
            <Typography>
              <strong>Date:</strong> {request.date}
            </Typography>
            <Typography>
              <strong>Amount:</strong> {request.amount} LE
            </Typography>
            <Typography>
